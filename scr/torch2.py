...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load('./trainedModel4Epochs.pth'))
    net.eval()
    ...
    with torch.no_grad():
        for i in tqdm(range(len(X_test))):
            ...
            real_class_str = str(int(real_class))
            predict_class_str = str(int(predict_class))

            if predict_class == real_class:
                correct += 1
                if ( predict_class_str not in predict_dict.keys()):
                    predict_dict[ predict_class_str] = 1
                else:
                    predict_dict[predict_class_str] += 1

            total += 1

            if ( real_class_str not in real_dict.keys()):
                real_dict[ real_class_str] = 1
            else:
                real_dict[ real_class_str] += 1
...
r = sorted(real_dict.items())
p = sorted(predict_dict.items())
predict_diff = []
for i in range(len(r)):
    predict_diff.append(r[i][1] - p[i][1])

print("Category wise error in prediction (real - predict) !")
print('Label')
print(list(range(10)))
print('predict differene')
print(predict_diff)
